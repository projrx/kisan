...
<head>
	...
	<!-- Document Title
	============================================= -->
	<title>Donate Zakat  | APKI Party</title>
...
<?php 
  if(isset($_POST['updateid'])){
  	...
  	$datec=date('Y-m-d');
  	$data=mysqli_query($con,"INSERT INTO zakat 
    ...
    , '".mysqli_real_escape_string($con,$_POST['fname'])."'
      ...
      $rows =mysqli_query($con,"SELECT id FROM zakat ORDER BY id desc limit 1" ) or die(mysqli_error($con));
       while($row=mysqli_fetch_array($rows)){
        $id = $row['id']; 	}

      ($msg=mysqli_error($con));
      if(empty($msg)) header("location:zakatfund.php?success=1&memberid=$id");
      ...
      <tbody>
        <form method="post" action="" enctype="multipart/form-data">
          ...
          <tr style="background: none !important;">	<td colspan="2">
	          		...
	          		<lablel> I ' <input style="border: none;width:auto;" class="" type="text" id="tname" name="tname" value="">' giving this fund (which is earned from legal source)  to APKI for the purpose of Zakat foundation.
	          		</label>
          		</td>
		...
		<section>
			<div class="container">
				<div class="text-center">
					...
					Transaction successful we appreciate your effort to support needy persons. .<br>
					Your Zakat Transaction id is: <strong style="font-weight: 800"> <?php echo $_GET['memberid'] ?> </strong> .<br>
